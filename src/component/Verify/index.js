...
const Verify = ()=>{
    ...
    const handleClick = ()=>{
        setLoading(true)
        axios.post('https://portal-auth-service-latest.onrender.com/auth/v1/user/verify-user',{
          email:email,
          token:token,
        },{
            headers:{
    ...
    return(
        <div>
            ...
            <div >
                <label>token :</label>
                <textarea style={{width:'300px'}} value={token} onChange={(e)=>{setToken(e.target.value)}}></textarea>
            </div>
